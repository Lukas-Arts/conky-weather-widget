...
# every 10 mins, also update the satellite image

source "$HOME/.conky/weather-widget/settings.properties"
weatherinfo=""
last_update=0
...
if ((my_now-last_update < 300)) then
    ...
    #echo "using $1"
    if [ "$weather_provider" == "wttr" ]; then
        get_and_crop_radar_log=$($HOME/.conky/weather-widget/get_and_crop_radar_image.sh "$region_name" "$detail_x" "$detail_y")

        #echo "using wttr"
        weatherinfo=$(curl  -H "Accept-Encoding: gzip,deflate" -s "https://wttr.in/$location_name?format=j1&lang=de")
        printf "$weatherinfo" > "$HOME/.conky/weather-widget/weatherinfo.json"
    elif [ "$weather_provider" == "open-meteo" ]; then
        get_and_crop_radar_log=$($HOME/.conky/weather-widget/get_and_crop_radar_image.sh "$region_name" "$detail_x" "$detail_y")

        #echo "using open-meteo"
        url="https://api.open-meteo.com/v1/forecast?latitude=$lat&longitude=$lon&daily=weather_code,temperature_2m_max,temperature_2m_min,apparent_temperature_max,apparent_temperature_min,sunshine_duration,daylight_duration,rain_sum,showers_sum,snowfall_sum,precipitation_sum,precipitation_hours,precipitation_probability_max,wind_speed_10m_max,wind_gusts_10m_max,wind_direction_10m_dominant,temperature_2m_mean,apparent_temperature_mean,precipitation_probability_mean,sunrise,sunset,relative_humidity_2m_mean,visibility_mean,wind_speed_10m_mean,wind_gusts_10m_mean,surface_pressure_mean&hourly=precipitation_probability,weather_code,relative_humidity_2m,apparent_temperature,dew_point_2m,cloud_cover,visibility,is_day,temperature_2m&current=weather_code,temperature_2m,relative_humidity_2m,apparent_temperature,is_day,precipitation,cloud_cover,wind_speed_10m,wind_direction_10m,wind_gusts_10m,pressure_msl,surface_pressure&minutely_15=temperature_2m,relative_humidity_2m,apparent_temperature,sunshine_duration,weather_code,visibility,is_day&timezone=Europe%2FBerlin&forecast_minutely_15=96"
