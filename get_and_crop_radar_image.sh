...
target_x=192
timestamp_part_file="$HOME/.conky/weather-widget/gif_frames_edit/timestamp_part_${filename}"
...
if [ "$1" == "hes" ]; then
    echo "Cropping Part $img -> $timestamp_part_file"
    convert "$img" -crop "230x21+${crop_x}+${crop_y}" +repage "$timestamp_part_file"
    echo "Adjusting transparency on part: $timestamp_part_file"
    convert "$timestamp_part_file" -alpha set -channel A -evaluate Multiply $opacity +channel "$timestamp_part_file"
fi

# crop & resize detail view if $2 is specified
if [ -n "$2" ]; then
    echo "Cropping Detail-Part $img at ${2}/${3} -> $detail_part_file"
...
if [ "$1" == "hes" ]; then
    ...
    echo "Compositing (+${pos_x}/+${pos_y}) $timestamp_part_file onto $img -> $comp_file "

    convert "$img" "$timestamp_part_file" -geometry +${pos_x}+${pos_y} -composite "$comp_file"

    # add detail view if $2 is specified
    if [ -n "$2" ]; then
        echo "Compositing Detail-Part (+140/+0) $detail_part_file onto $comp_file -> $comp_file "

        convert "$comp_file" "$detail_part_file" -geometry +140+0 -composite "$comp_file"
    fi

    echo "Cropping $comp_file -> $cropped_file"
    convert "$comp_file" -crop "400x400+140+0" +repage "$cropped_file"
else
    # add detail view if $2 is specified
    if [ -n "$2" ]; then
        echo "Compositing Detail-Part (+0/+0) $detail_part_file onto $img -> $cropped_file "
...
echo "Adjusting transparency for final: $cropped_file -> $final_file"
